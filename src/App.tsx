...
const LoadingScreen = () => {
  ...
  return (
    <motion.div 
      ...
      className="fixed inset-0 z-[100] bg-brand-black flex flex-col items-center justify-center p-6 text-brand-green font-mono overflow-hidden"
    >
      <div className="absolute inset-0 grid-bg opacity-10" />
      <div className="relative z-10 w-full max-w-md">
        <div className="flex items-center gap-4 mb-8">
          <div className="w-12 h-12 border-2 border-brand-green animate-spin flex items-center justify-center">
            <div className="w-6 h-6 bg-brand-green animate-pulse" />
          </div>
          <div className="flex flex-col">
            <span className="text-xl font-display font-black tracking-tighter text-white">CODESAPIENS</span>
            <span className="text-[10px] tracking-[0.3em] font-bold">BOOT_SEQUENCE_V2.6</span>
          </div>
        </div>
        ...
        <div className="space-y-2 h-40 overflow-hidden">
          ...
          <motion.div 
            ...
            className="w-2 h-4 bg-brand-green inline-block ml-1 align-middle"
          />
        </div>

        <div className="mt-8 w-full h-1 bg-white/10 relative overflow-hidden">
          <motion.div 
            initial={{ width: 0 }}
            animate={{ width: '100%' }}
            transition={{ duration: 2.3, ease: "easeInOut" }}
            className="absolute top-0 left-0 h-full bg-brand-green shadow-[0_0_10px_#00FF00]"
          />
        </div>
      </div>
    </motion.div>
...
export default function App() {
  ...
  const navigateToNextTrack = () => {
    if (!selectedTrackId) return;
    const currentIndex = TRACKS_FLOW.findIndex(t => t.id === selectedTrackId);
    const nextIndex = (currentIndex + 1) % TRACKS_FLOW.length;
    setSelectedTrackId(TRACKS_FLOW[nextIndex].id);
    window.scrollTo({ top: 0, behavior: 'smooth' });
  };

  const navigateToPrevTrack = () => {
    if (!selectedTrackId) return;
    const currentIndex = TRACKS_FLOW.findIndex(t => t.id === selectedTrackId);
    const prevIndex = (currentIndex - 1 + TRACKS_FLOW.length) % TRACKS_FLOW.length;
    setSelectedTrackId(TRACKS_FLOW[prevIndex].id);
    window.scrollTo({ top: 0, behavior: 'smooth' });
  };
  ...
  const navLinks = [
    { label: 'HOME', href: '#home', action: () => { setView('home'); window.scrollTo({ top: 0, behavior: 'smooth' }); } },
    { label: 'TRACKS', href: '#schedule', action: navigateToSelectPath },
    { label: 'AGENDA', href: '#schedule', action: () => navigateToHomeAndScroll('#schedule') },
    { label: 'COMMUNITY', href: '#social', action: () => navigateToHomeAndScroll('#social') },
  ];
      ...
      <div className="min-h-screen bg-brand-black text-white font-sans overflow-x-hidden relative">
        ...
        <nav className="fixed top-0 left-0 right-0 z-50 px-6 py-6 flex justify-between items-center bg-brand-black/20 backdrop-blur-md border-b border-white/5">
          <div className="flex items-center gap-3 cursor-pointer" onClick={navigateToSelectPath}>
            <img src="/input_file_22.png" alt="CSF Logo" className="w-10 h-10 object-contain" referrerPolicy="no-referrer" />
            <div className="flex flex-col">
          ...
          <div className="hidden md:flex gap-10 text-[10px] font-mono uppercase tracking-[0.2em] text-white/40">
             {navLinks.map(link => (
               <button key={link.label} onClick={() => link.action && link.action()} className="hover:text-brand-green transition-colors uppercase cursor-pointer">{link.label}</button>
             ))}
          </div>
      ...
      <div className="min-h-screen bg-brand-black text-white font-sans overflow-x-hidden relative">
        ...
        <section className="pt-40 pb-20 px-6 max-w-4xl mx-auto flex flex-col items-center min-h-screen">
          ...
          <div className="w-full text-center mb-16 px-4">
            <div className="flex items-center justify-center gap-4 md:gap-12 mb-4">
              <button 
                onClick={navigateToPrevTrack}
                className="p-2 md:p-4 rounded-full border border-white/5 bg-white/5 hover:bg-brand-green hover:text-black hover:border-brand-green transition-all group flex items-center justify-center"
                title="Previous Track"
              >
                <ChevronLeft size={24} className="group-hover:scale-125 transition-transform" />
              </button>

              <h1 className={`text-5xl md:text-8xl font-display font-black tracking-tighter ${selectedTrack.textColor}`}>
                {selectedTrack.name}
              </h1>

              <button 
                onClick={navigateToNextTrack}
                className="p-2 md:p-4 rounded-full border border-white/5 bg-white/5 hover:bg-brand-green hover:text-black hover:border-brand-green transition-all group flex items-center justify-center"
                title="Next Track"
              >
    ...
    <AnimatePresence mode="wait">
      {isLoading ? (
        ...
        <motion.div 
          ...
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          className="min-h-screen bg-brand-black overflow-x-hidden font-sans"
        >
          <Stars />
          ...
          <section id="home" className="relative h-screen flex flex-col items-center justify-center px-6 overflow-hidden pt-20">
            ...
            <motion.div 
              ...
              initial="hidden"
              animate="visible"
              className="text-center space-y-6 relative z-10 mb-12"
            >
              <motion.h1 
              ...
              >
                <span className="flex items-center gap-2">SUMMER <img src="/input_file_22.png" className="w-12 md:w-20 h-auto animate-spin-slow filter invert" alt="logo" referrerPolicy="no-referrer" /></span>
      ...
      <section id="sponsors" className="py-24 bg-white/5 border-t border-white/10">
        <div className="max-w-7xl mx-auto px-6">
          ...
          <motion.div 
            ...
            whileInView="visible"
            viewport={{ once: true }}
            className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-[1px] bg-white/10 border border-white/10"
          >
            {[
              { name: 'Google Student Ambassador', img: '/gsa-logo.jpeg' },
              { name: 'Namma Flutter', img: '/namma-flutter-logo.jpeg' },
              { name: 'Nexora', img: '/nexora-logo.jpeg' },
              { name: 'Microsoft Learn Student Ambassador', img: '/microsoft-ambassador-logo.png' },
              { name: 'CSF', img: '/input_file_22.png' }
            ].map((partner, i) => (
              <motion.div 
                key={i} 
                variants={itemVariants}
                className="h-40 bg-brand-black flex items-center justify-center p-6 transition-all duration-300 group cursor-crosshair border border-white/5"
              >
                <img 
                  src={partner.img} 
                  alt={partner.name} 
                  className="max-w-[70%] max-h-[70%] object-contain opacity-80 group-hover:opacity-100 transition-all" 
                  referrerPolicy="no-referrer"
                />
              </motion.div>
            ))}
      ...
      <footer className="py-12 px-6 border-t border-white/5 bg-brand-black">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-8 text-[9px] font-mono tracking-widest uppercase text-white/40">
           <div className="flex items-center gap-4">
             <img src="/input_file_22.png" alt="" className="w-8 h-8 object-contain opacity-80" referrerPolicy="no-referrer" />
